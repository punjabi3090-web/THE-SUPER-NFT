...
const plans = [
  ...
  { id: "30d", label: "30 Days", apy: 12, apyStr: "12%", days: 30, color: "from-blue-400 to-indigo-500",   bgLight: "from-blue-50 to-indigo-50",   border: "border-blue-200",   tag: "Popular" },
  { id: "90d", label: "90 Days", apy: 25, apyStr: "25%", days: 90, color: "from-purple-400 to-pink-500",   bgLight: "from-purple-50 to-pink-50",   border: "border-purple-200", tag: "Best Return" },
];
...
export default function Stake() {
  ...
  const [stakes]                        = useState<Stake[]>(() => getStakes());
  const [selectedPlan, setSelectedPlan] = useState(plans[0]);
  const [amount, setAmount]             = useState("");
  const [showToast, setShowToast]       = useState(false);

  const numAmount   = parseFloat(amount || "0");
  const dailyProfit = numAmount > 0 ? (numAmount * selectedPlan.apy) / 100 / selectedPlan.days : 0;
  const totalReturn = numAmount > 0 ? numAmount + (numAmount * selectedPlan.apy) / 100 : 0;
  ...
  return (
    <div className="pb-20 max-w-md mx-auto">
      <Header />
      {TEST_MODE && (
        <div className="bg-yellow-100 text-yellow-800 text-xs text-center py-1 font-medium">🧪 Test Mode</div>
      )}

      {showToast && (
        <div className="fixed top-4 left-1/2 -translate-x-1/2 z-50 bg-slate-700 text-white px-5 py-3 rounded-xl shadow-lg font-semibold text-sm">
          🚧 Staking Coming Soon!
        </div>
      )}
      ...
      <div className="mx-4 mt-4 bg-white rounded-2xl p-4 shadow-sm">
        ...
        <input type="number" placeholder="Min $10 USDT" value={amount}
      ...
      {/* Stake button — Coming Soon */}
      <div className="px-4 mt-4">
        <button onClick={handleStakeClick}
          className={`w-full font-bold py-3.5 rounded-2xl shadow-lg text-white opacity-70 bg-gradient-to-r ${selectedPlan.color} relative overflow-hidden`}>
          <span className="opacity-50">Stake ${numAmount > 0 ? numAmount.toFixed(2) : "0.00"} → {selectedPlan.apy}% in {selectedPlan.label}</span>
          <span className="absolute inset-0 flex items-center justify-center font-extrabold text-base tracking-wide">
            🚧 Coming Soon
          </span>
        </button>
      </div>
            ...
            {stakes.map(s => (
              <div key={s.id} className="bg-white rounded-2xl p-4 shadow-sm flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-blue-500 mt-0.5">Until: {new Date(s.endDate).toLocaleDateString()}</p>
                </div>
