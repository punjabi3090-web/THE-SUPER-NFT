...
export default function WithdrawRecord() {
  ...
  const [records] = useState<HistoryItem[]>(() =>
    getHistory().filter(h => h.type === 'withdrawal')
  );
  ...
  return (
    <div className="min-h-screen max-w-md mx-auto bg-gray-50 pb-8">
      ...
      <div className="flex items-center gap-3 px-4 pt-4 pb-4 bg-white shadow-sm sticky top-0 z-10">
        ...
        <h1 className="text-lg font-bold text-[#1E293B]">Withdrawal Records</h1>
      ...
      <div className="px-4 mt-4">
        {records.length === 0 ? (
          <div className="text-center py-24 text-slate-400">
            ...
            <p className="font-semibold text-slate-500 mb-1">No withdrawal records found</p>
            <p className="text-sm">Your withdrawal requests will appear here</p>
          </div>
        ) : (
          <div className="space-y-3">
            {records.map(item => (
              <div key={item.id} className="bg-white rounded-2xl p-4 shadow-sm border-l-4" style={{ borderColor: '#EA4335' }}>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="text-right">
                    {item.amount !== undefined && (
                      <p className="font-bold text-[#EA4335]">-${Number(item.amount).toFixed(2)}</p>
                    )}
                    {item.status && (
