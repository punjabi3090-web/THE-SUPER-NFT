...
import { getUserTeam, getCurrentUserId, type User } from "../lib/api";
...
export default function MyTeam() {
  ...
  useEffect(() => {
    if (!uid) return;
    getUserTeam(uid).then(r => { setTeam(r.team); setLoading(false); }).catch(() => setLoading(false));
  }, [uid]);
    ...
    <div className="pb-20 max-w-md mx-auto">
      ...
      <div className="px-4 pt-4">
        <div className="flex items-center gap-3 mb-4">
          <button onClick={() => setLocation('/my')} className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ background: '#EFF6FF' }}>
            <ArrowLeft size={18} style={{ color: '#1E3A8A' }} />
          </button>
          <h1 className="text-xl font-bold text-slate-800">My Team</h1>
          <span className="ml-auto bg-blue-100 text-blue-700 text-xs font-bold px-3 py-1 rounded-full">{team.length} Members</span>
        </div>
        ...
        <div className="bg-white rounded-2xl p-4 shadow-sm mb-4" style={{ borderLeft: '4px solid #1E3A8A' }}>
          ...
          <span className="text-2xl font-black tracking-widest" style={{ color: '#1E3A8A' }}>{user?.referralCode || '—'}</span>
          <p className="text-xs text-slate-400 mt-3 mb-2 font-medium">Your Invite Link</p>
        ...
        <div className="grid grid-cols-3 gap-3 mb-4">
          {[
            { label: 'Total Members', value: team.length.toString(), color: 'text-blue-600', icon: '👥' },
            { label: 'Team Deposits', value: `$${totalTeamDeposit.toFixed(2)}`, color: 'text-emerald-600', icon: '💰' },
            { label: 'Team Balance', value: `$${totalTeamBalance.toFixed(2)}`, color: 'text-purple-600', icon: '💎' },
          ].map((s, i) => (
        ...
        {/* Team List */}
        {loading ? (
          <div className="text-center py-12 text-slate-400"><p className="text-2xl mb-2">⏳</p><p className="text-sm">Loading team...</p></div>
        ) : team.length === 0 ? (
          <div className="bg-white rounded-2xl p-10 shadow-sm text-center">
            ...
            <button onClick={copyLink} className="px-6 py-2.5 rounded-xl text-white text-sm font-bold" style={{ background: '#1E3A8A' }}>
              📋 Copy Invite Link
            </button>
          </div>
        ) : (
          <div className="space-y-3">
            <p className="text-xs text-slate-400 font-semibold uppercase tracking-wide px-1">Direct Referrals ({team.length})</p>
            {team.map(m => (
              <div key={m.id} className="bg-white rounded-2xl p-4 shadow-sm flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-slate-300 mt-0.5">Joined {new Date(m.joinDate).toLocaleDateString()}</p>
                </div>
